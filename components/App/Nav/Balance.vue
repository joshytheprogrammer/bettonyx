<template>
  <div class="flex items-center pr-4">
    <NuxtLink to="/account/my/wallet" class="mr-4 font-bold text-sm md:text-lg border-b-2 border-secondary-400 text-white">
      {{ 
    ...
    <UTooltip text="Reload" :popper="{ arrow: true }">
      <button @click="reloadBalance" :disabled="loading" class="focus:outline-none">
        <svg v-if="!loading" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v6h6M20 20v-6h-6M4 4l16 16" />
        </svg>
        <svg v-else xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 animate-spin text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
